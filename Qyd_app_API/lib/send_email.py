...
def send_mail_report(title):
    ...
    """获取最新测试报告"""
    report_path = config.basedir+"/report/"
    new_report = ""
    ...
    zip_report(r"./", './', 'QYD_API_Test_Report.zip')
    ...
    """将测试报告压缩文件添加到邮件附件"""
    att = MIMEText(open('./QYD_API_Test_Report.zip', 'rb').read(), 'base64', 'utf-8')
    att["Content-Type"] = 'application/octet-stream'
    att.add_header("Content-Disposition", "attachment", filename="QYD_API_Test_Report.zip")
    msg.attach(att)
