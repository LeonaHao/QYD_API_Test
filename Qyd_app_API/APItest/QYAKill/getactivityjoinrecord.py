...
#        print(headers)
    ...
    r = requests.post(url.getactivityjoinrecord_QA, json=body, headers=headers, verify=False)
    result = r.json()
    ...
    if result['code'] == 10000:
        logger.info("————————————————轻盈A秒杀加入记录接口返回成功————————————————")
        logger.info("轻盈A秒杀活动购买页展示接口返回信息：" + result_dump)
        assert(result['msg'] == '成功')
        assert(result['successful'] == True)
        if len(result['data']) == 0:
            logger.info("加入记录暂无数据")
    else:
        logger.info("————————————————轻盈A秒杀加入记录接口返回失败————————————————")
